...
def main_page():
    """Main EMR Log Analytics page"""
    # Status indicator
    if state.analysis_complete:
        st.success("✅ Analysis Ready")
    elif state.uploaded_logs:
    ...
    with st.expander("📐 System Architecture", expanded=False):
        try:
            st.image(
                ...
                caption="EMR Log Analytics System Architecture",
                use_container_width=True
            )
        except Exception as e:
            st.error(f"Could not load architecture diagram: {e}")
    
    with st.expander("🔧 EMR Analyzer Component", expanded=False):
        try:
            st.image(
                "images/emr_analyzer.png",
                caption="EMR Analyzer Component Architecture",
                use_container_width=True
            )
        except Exception as e:
            st.error(f"Could not load EMR analyzer diagram: {e}")
