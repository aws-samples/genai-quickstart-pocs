...
async def main():
    ...
    if st.session_state.stage == "input":
        ...
        if st.button("Generate", key="generate_button"):
            ...
            parameters, missing_params = await extractor.extract_parameters(
                prompt, template
            )
            progess_bar.progress(50, text="Mapping data to parameter values...")
            parameter_values = await extractor.map_parameter_values(
                prompt, template, parameters
            )
            progess_bar.progress(75, text="Generating overview...")
            description = await extractor.generate_description(
                prompt, template, missing_params
            )
        ...
        if st.session_state.missing_params:
            ...
            st.write(
                "The following parameters are able to be added to improve the image. A default will be assigned if no value is provided."
            )
            params = {}
            logger.trace("Missing params", st.session_state.missing_params)
            for param_name, param_info in st.session_state.missing_params.items():
                if param_info["type"] == "number":
                    params[param_name] = st.number_input(
                        param_name, key=f"param_num_{param_name}"
                    )
                elif param_info["type"] == "boolean":
                    params[param_name] = st.checkbox(
                        param_name, key=f"param_bool_{param_name}"
                    )
                else:
                    params[param_name] = st.text_input(
                        param_name, key=f"param_str_{param_name}"
                    )
            if st.button("Generate with Parameters"):
                st.session_state.parameters.update(params)
                logger.debug(f"Updated parameters:\n{st.session_state.parameters}")
                st.session_state.missing_params = None
                st.rerun()
        else:
            with st.spinner("Generating image..."):
                request = ImageRequest(
                    prompt=st.session_state.prompt, parameters=st.session_state.parameters
                )
                st.session_state.function_id = generator._get_function_id(
                    request, st.session_state.template
                )
                image_data, image_type = generator.run_image_generation(
                    request, st.session_state.template, st.session_state.parameter_values
                )
                st.session_state.current_image = image_data
                st.session_state.current_image_type = image_type
                logger.trace("Image type", image_type)
        ...
        for feedback in st.session_state.previous_feedback:
            st.write(f"* {feedback}")
        st.image(
            f"data:{st.session_state.current_image_type};base64,{st.session_state.current_image}"
        )
