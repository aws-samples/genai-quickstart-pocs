...
from .redshift_connector import get_detailed_schema_info
...
def extract_sql_from_response(response_json):
    ...
    try:
        ...
        # Fix Redshift syntax: replace single quotes with double quotes for identifiers
...
def fallback_sql_query(nl_query):
    ...
    Returns:
        list: List of SQL queries.
    """
    # Simplified fallback logic for common cases using Northwind schema
    if "list tables" in nl_query.lower():
        return ["SELECT table_name FROM information_schema.tables WHERE table_schema = 'northwind';"]
    elif "sample records" in nl_query.lower():
        return ["SELECT * FROM northwind.customers LIMIT 5;"]
    elif "count" in nl_query.lower() and "orders" in nl_query.lower() and "country" in nl_query.lower():
        return ["""
...
def generate_data_summary(database, schema, topic, limit=1000):
    ...
    Returns:
        str: Data summary.
    """
    from .redshift_connector import get_detailed_schema_info, analyze_table_relationships, connect_to_redshift
    ...
    try:
        ...
        if not database.startswith('REDSHIFT_SAMPLE'):
