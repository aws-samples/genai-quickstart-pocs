...
This Python implementation demonstrates real-time audio streaming integration with AWS Bedrock's Nova Sonic model. It enables natural conversational interactions where customers can verbally provide information to file car insurance claims. The sample demonstrates toolUse integration with the Amazon Nova Sonic model for collecting comprehensive claim information and automatically generating and outputting professional PDF claim forms.

![A gif of a screen recording show casing the Amazon Bedrock Model Playground POC functionality](images/ScreenRecordingFinal(D)runihouytfdhrfg.gif)
...
The architecture and flow of the POC is as follows:
![POC Architecture & Flow](images/NSarch.png 'POC Architecture')
...
1. Clone the repository to your local machine.
    ...
    * `insurance_claim_agent_demo.py` - The main application for the insurance claim system
    * `insurance_claim_system/` - Python code that simulates the insurance claim system with components for claim processing and PDF generation
    * `generated_claims/` - The directory that will store any generated claims
    * `test/` - Holds the file to test the pdf claim generator tool
    * `images/` - Holds the images for this readme
    * `HOWTO.md` - shows the detailed instructions on using this implementation
...
```bash
python insurance_claim_agent_demo.py
# Or with debug mode
python insurance_claim_agent_demo.py --debug
```
