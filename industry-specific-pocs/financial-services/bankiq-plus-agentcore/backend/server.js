...
app.get('/api/diagnostics', (req, res) => {
  ...
  if (!agentRuntimeArn) {
    throw new Error('AGENTCORE_AGENT_ARN environment variable not set');
  }
...
async function invokeAgent(req, res) {
  ...
  try {
    ...
    const agentRuntimeArn = process.env.AGENTCORE_AGENT_ARN; if (!agentRuntimeArn) throw new Error("AGENTCORE_AGENT_ARN not set");
...
app.post('/api/get-sec-filings', async (req, res) => {
  ...
  try {
    ...
    logger.info(`Found ${tenK.length} 10-K and ${tenQ.length} 10-Q filings for ${bankName} (2023-2025)`);
    logger.debug(`10-K filings: ${tenK.map(f => f.filing_date).join(', ')}`);
    logger.debug(`10-Q filings: ${tenQ.map(f => f.filing_date).join(', ')}`);
  ...
  try {
    ...
    if (results.length === 0) {
      try {
        ...
        logger.info(`Found ${results.length} companies in SEC EDGAR matching "${query}"`);
      } catch (edgarError) {
        logger.error('SEC EDGAR search failed:', edgarError.message);
      }
    } else {
      logger.info(`Found ${results.length} banks in local cache matching "${query}"`);
    }
