    ...
    "inputSchema": {
        "json": {
            ...
            "properties": {
                ...
                "status": {
                    "type": "string",
                    "description": "Filter by redemption status (e.g., Approved, Rejected, Pending)"
                },
                "start_date": {
                    "type": "string",
                    "description": "Start date filter for request date (YYYY-MM-DD format)"
                },
                "end_date": {
                    "type": "string",
                    "description": "End date filter for request date (YYYY-MM-DD format)"
                },
                "limit": {
                    "type": "integer",
...
def pull_s3_data___get_redemption_requests(tool, **kwargs: Any):
    ...
    try:
        s3_client = boto3.client('s3', region_name='us-east-1')
        s3_key = "database/redemption_requests.csv"
        ...
        for row in csv_reader:
            ...
            if start_date and row.get('request_date', '') < start_date:
                continue
                
            if end_date and row.get('request_date', '') > end_date:
                continue
        ...
        return {
            "toolUseId": tool_use_id,
            "status": "error",
            "content": [{"text": f"Redemption requests data not found: {s3_key} in bucket {bucket}"}]
        }
