...
const ChatContainer: React.FC<ChatContainerProps> = ({ userName }) => {
  ...
  // Version that accepts specific personality and session ID (for personality changes)
  const handleSubmitHiddenWithPersonality = async (message: string, personality: string, sessionIdToUse: string) => {
    if (!message.trim() || isLoading) return;
  ...
  const handlePersonalityChange = (newPersonality: string) => {
    ...
    // Send Hello after state updates (hidden from user) - pass new personality directly
    setTimeout(() => {
      console.log('Sending Hello from personality change with personality:', newPersonality);
      handleSubmitHiddenWithPersonality('Hello', newPersonality, newSessionId);
    }, 100);
  };
