...
const mockAuth = (req: AuthenticatedRequest, res: Response, next: express.NextFunction) => {
...
// Mock API endpoints
app.post('/api/v1/ideas/requests', mockAuth, (req: AuthenticatedRequest, res: Response) => {
...
app.get('/api/v1/ideas/requests/:requestId/status', mockAuth, (req: AuthenticatedRequest, res: Response) => {
...
app.get('/api/v1/ideas/requests/:requestId/results', mockAuth, (req: AuthenticatedRequest, res: Response) => {
  const { requestId } = req.params;
  const request = mockRequests.get(requestId);
...
app.delete('/api/v1/ideas/requests/:requestId', mockAuth, (req: AuthenticatedRequest, res: Response) => {
  const { requestId } = req.params;
  const request = mockRequests.get(requestId);
...
app.get('/api/v1/ideas/requests', mockAuth, (req: AuthenticatedRequest, res: Response) => {
  ...
  res.json({
    ...
    pagination: {
      ...
      totalPages: Math.ceil(userRequests.length / parseInt(limit as string))
    }
  });
});

app.post('/api/v1/ideas/requests/:requestId/feedback', mockAuth, (req: AuthenticatedRequest, res: Response) => {
  const { requestId } = req.params;
  const request = mockRequests.get(requestId);
...
function generateMockResults(parameters: any) {
