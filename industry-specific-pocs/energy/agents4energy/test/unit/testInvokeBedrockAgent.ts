import { handler } from "@/../amplify/functions/invokeBedrockAgent"
import { AppSyncResolverEvent, Context, AppSyncIdentity } from 'aws-lambda';
import { Schema } from '@/../amplify/data/resource';
import { STSClient } from "@aws-sdk/client-sts";
import { AmplifyClientWrapper, createChatMessage } from '@/../amplify/functions/utils/amplifyUtils'
import { ChatMessageRole } from '@/../amplify/functions/graphql/API'
...
export const main = async () => {
    ...
    await getLambdaEnvironmentVariables(await getDeployedResourceArn(rootStackName, 'invokebedrockagentlambda'))
    ...
    const testChatMessage = await amplifyClientWrapper.amplifyClient.graphql({
        query: createChatMessage,
        variables: {
            input: {
                chatSessionId: testChatSession.data.createChatSession.id,
                content: 'How many tanks are in my biodiesel unit?',
    ...
    const testArguments = {
        ...
        chatSessionId: testChatSession.data.createChatSession.id,
        prompt: 'How many tanks are in my biodiesel unit?',
    }

    const event: AppSyncResolverEvent<Schema['invokeBedrockAgent']['args']> = {
